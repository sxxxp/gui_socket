defmodule Gui.Scene.MainChat do
  ...
  @info "hi! welcome to Socket Server!
      IP address: #{@host}
      Port: #{@port}"
  @graph Graph.build(font: :roboto, font_size: @text_size)|> add_specs_to_graph([
      ...
      |> rect( {50, 30}, fill: :light_gray,id: :submit_button,input: :cursor_button, translate: {330, 240})
  ...
  def handle_input({:cursor_button, {:btn_left, 1,_, _}}, :submit_button,%{assigns: %{chat: chat,log: log,focus: focus}} = scene) do
    if chat != "" do
  ...
  def handle_event({:value_changed, :text_id, new_value}, _context, %{assigns: %{chat: chat}} = scene) do
    chat = new_value
